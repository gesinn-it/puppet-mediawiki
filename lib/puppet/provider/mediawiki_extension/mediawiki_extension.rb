...
#  defaultfor :operatingsystem => [:CentOS, :RedHat]
  ...
  def create
    # Fetch code to tmp
    curl('-R', '-z', "/vagrant_share/tmp/#{name}.tar.gz", '-o', "/vagrant_share/tmp/#{name}.tar.gz", "#{source}")
    # Make deploy dir
    ...
    tar('-xzf', "/vagrant_share/tmp/#{name}.tar.gz", '-C', "#{doc_root}/#{instance}/extensions/#{name}")
    # sync db
